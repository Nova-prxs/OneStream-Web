...
Namespace OneStream.BusinessRule.Connector.PLP_PeoplePlanningData_EntityTIn
    Public Class MainClass
        '-------------------------------------------------------------------------------------------------------------------
        'ENTITY TRANSFER-IN CONNECTOR
        '
        'Load and process plan data from other workflows transferring to an entity owned by the executing Workflow
        '
        'Usage Requirements:
        '
        '1) Assumes worflows have assigned entities
        '2) Intended to be used for distributed People Plan Register that require cross Workflow entity transfers
        '3) This Connector should be assigned to the Transfer-In Workflow
        '4) End Users should execute the Import-Validate-Load Workflow in order to review Employees being transferred in
        '
        '--------------------------------------------------------------------------------------------------------------------
        'PEOPLE PLANNING CONNECTOR (DataSource Configuration Requirements)
        '
...
#Region "Connection and Field List Methods"
        ...
        Private Function GetFieldListSQL(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs) As String
            Try
                ...
                sql.Append("Select Top 1 Status, WFProfileName, WFScenarioName, WFTimeName, Period, Entity, ActivityType, ImpactType, Account, Flow, IC, UD1, UD2, UD3, UD4, UD5, UD6, UD7, UD8, Amount As Amt From XFW_PLP_Plan")
                Return sql.ToString
...
#Region "Source Data Methods"
        ...
        Private Function GetDataSelectSQL(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs) As String
            Try
                'Retrieve the People Plan scenario and time to load from stored Settings
                Dim plpScenario As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "StoredPlanConnectorScenario_PLPT")
                Dim plpTime As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "StoredPlanConnectorTime_PLPT")

                If plpScenario.XFEqualsIgnoreCase("WF") Then plpScenario = ScenarioDimHelper.GetNameFromId(si, api.WorkflowUnitPk.ScenarioKey)
                If plpTime.XFEqualsIgnoreCase("WF") Then plpTime = BRApi.Finance.Time.GetNameFromId(si, api.WorkflowUnitPk.TimeKey)

                'Execute a summary query against the Plan Table
                Dim sql As New Text.StringBuilder

                'Query unrelated workflows that have create plan data for an entity assigned to the Workflow profile executing the Transfer-In Load
                sql.Append("Select Status, WFProfileName, WFScenarioName, WFTimeName, Period, Entity, ActivityType, ImpactType, Account, Flow, IC, UD1, UD2, UD3, UD4, UD5, UD6, UD7, UD8, Sum(Amount) As Amt ")
                sql.Append("From XFW_PLP_Plan ")
                sql.Append("Where ")
                sql.Append("(WFScenarioName = '" & SqlStringHelper.EscapeSqlString(plpScenario) & "') ")
                sql.Append("And (WFTimeName = '" & SqlStringHelper.EscapeSqlString(plpTime) & "') ")
                sql.Append(Me.GetUnrelatedWFProfileCriteria(si, api))
        ...
        Private Sub ValidateNoImportFromCentralRegisterProfile(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs)
            Try
                ...
                Dim registerProfile As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "StoredPlanRegisterProfile_PLPT")
                ...
                Dim wfProfileCriteria As Integer = CType(BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "StoredPlanConnectorProfile_PLPT"), Integer)
                ...
                If registerProfile.XFEqualsIgnoreCase(loadingProfile) Then
                    ...
                    Throw New Exception("People Planning: Cannot load data into cube from the Central Register Mangement Workflow Profile '" & registerProfile & "'.")
                End If
...
#Region "Drill Back Methods"
        ...
        Private Function GetDrillBackTypeList(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs) As List(Of DrillBackTypeInfo)
            Try
                ...
                ElseIf args.DrillCode.XFEqualsIgnoreCase("RegisterIDDetail") Then
            ...
            Try
                Select Case drillBackType
                    ...
                    Case Is = ConnectorDrillBackDisplayTypes.DataGrid
                        ...
                        ElseIf args.DrillBackType.NameAndDescription.Name.XFEqualsIgnoreCase("Plan Trend (Forward 12)") Then
                            'Level 1: Plan Trend
                        ...
                        ElseIf args.DrillBackType.NameAndDescription.Name.XFEqualsIgnoreCase("All Plan Activity") Then
                        ...
                        ElseIf args.DrillBackType.NameAndDescription.Name.XFEqualsIgnoreCase("Calculation Plan (Calculation Detail)") Then
        ...
        Private Function GetDrillBackSQL_PlanDetail_L1(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs) As String
            Try
                ...
                If (Not sourceValues Is Nothing) And (sourceValues.Count > 0) Then
                    ...
                    fromClause.Append(" FROM XFW_PLP_Plan ")
        ...
        Private Function GetDrillBackSQL_AllPlanActivity_L2(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Transformer, ByVal args As ConnectorArgs) As String
            Try
                ...
                If (Not sourceValues Is Nothing) And (sourceValues.Count > 0) Then
                    ...
                    whereClause.Append("And (")
                    whereClause.Append("Period = " & SqlStringHelper.EscapeSqlString(sourceValues.Item(StageTableFields.StageSourceData.DimTime).ToString) & " ")
                    whereClause.Append(") ")
            ...
            Try
                ...
                If (Not sourceValues Is Nothing) And (sourceValues.Count > 0) Then
                    ...
                    If status.XFEqualsIgnoreCase("Exception") Then
                        ...
                        fromClause.Append(" FROM XFW_PLP_RegisterDetail ")
                    ...
                    Else
                        ...
                        fromClause.Append(" FROM XFW_PLP_CalcPlanDetail ")
...
#Region "General Connector Helper Methods"
            ...
            Try
                ...
                If Not dataSource Is Nothing Then
                    ...
                    If matchingDims.Count = 1 Then
                        ...
                        criteria = sourceField & " = '" & sourceValues(masterDimName) & "' "
                    Else
                        'Throw and error, we cannot automatically determine drillback with Concatenated Source dimensions
                        Throw New Exception("People Planning: Dimension '" & masterDimName & "' is concatenated from multiple source fields standard drill back not supported (Connector must be customized)")
                    End If
                End If
                Return criteria
        ...
        Private Function GetUnrelatedWFProfileCriteria(ByVal si As SessionInfo, ByVal api As Transformer) As String
            Try
                ...
                Select Case wfProfileCriteriaType
                    ...
                    Case Is = wfProfileCriteriaType.ProfileText1Map, wfProfileCriteriaType.SameProfile
                        'Note: For Trans-In the profile must always use Text1 Map (Loading Profile Must be Excluded)
                        ...
                        If Not String.IsNullOrWhiteSpace(mappedProfileName) Then
                            Return "And (WFProfileName <> '" & mappedProfileName & "') "
                        Else
                            Throw New Exception("People Planning: Profile Name Map value not assigned to Text1 for WFProfile '" & api.WorkflowProfile.Name & "', Scenario Type: '" & ScenarioType.GetItem(api.ScenarioTypeID).Name & "'")
                        End If
                    Case Else
                        Throw New Exception("People Planning: Unknown Profile Criteria Type.")
                End Select
            ...
            Try
                ...
                If Not assignedEntities Is Nothing Then
                    ...
                    Else
                        ...
                        Throw New Exception("People Planning: Workflow Profile '" & api.WorkflowProfile.Name & "' must have assigned Entities for T-Out/T-In Processing.")
                    End If
                Else
                    'Throw and error, No Assigned entities (Required For Entity Transfer Processing)
                    Throw New Exception("People Planning: Workflow Profile '" & api.WorkflowProfile.Name & "' must have assigned Entities for T-Out/T-In Processing.")
                    ...
                    For Each pDim As ParserDimensionInfo In dataSource.Dimensions
                        If pDim.Name.XFEqualsIgnoreCase(requiredMasterDim) Then
                            ...
                            If Not assignedSourceField.XFEqualsIgnoreCase(requiredSourceField) Then
                                ...
                                Throw New Exception("People Planning: DataSource '" & dataSource.Name & "' requires that source field '" & requiredSourceField & "' be assigned to dimension '" & requiredMasterDim & "'.")
                            End If
                            Exit For
                        End If
                    Next
