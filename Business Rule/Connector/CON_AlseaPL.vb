...
Namespace OneStream.BusinessRule.Connector.CON_AlseaPL
...
'						-------------------------------------------------------------------------------------------------------------------------
						...
						Dim selectQuery As String = $"SELECT 
													     'Actual' As Scenario
													     ,PL.ceco AS Entity 
														 ,PL.accounting_account AS Account
													     ,CAST(SUBSTRING(CAST(period AS VARCHAR(7)), 5, 3) AS INT) + (CAST(SUBSTRING(CAST(period AS VARCHAR(7)), 1, 4) AS INT) * 100) AS Period 
													     ,CASE WHEN LEFT(CAST(PL.accounting_account AS VARCHAR(8)), 1) = '6' THEN PL.accumulated_balance
															   ELSE PL.accumulated_balance END AS Amount
													FROM XFC_PL PL
													WHERE ROUND((period / 1000),0) = {wfYear}
													AND Company NOT IN ('Q001','Q029','Q031','Q032')"
