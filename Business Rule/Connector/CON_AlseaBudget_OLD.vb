...
Namespace OneStream.BusinessRule.Connector.CON_AlseaBudget_OLD
...
'						-------------------------------------------------------------------------------------------------------------------------
						...
						Dim selectQuery As String = $"
							WITH MonthlyCebeProportions AS (
							    ...
							    FROM XFC_RawSales rs WITH(NOLOCK)
							    INNER JOIN XFC_ChannelHierarchy ch WITH(NOLOCK) ON rs.cod_channel3 = ch.cod_channel3
							    INNER JOIN XFC_CEBES c WITH(NOLOCK) ON rs.unit_code = c.cebe
							    WHERE
							        ch.desc_channel1 = 'Delivery'
							        AND YEAR(rs.date) BETWEEN {wfYear - 2} AND {wfYear}
							        AND (
							    ...
							    FROM XFC_DailySales ds WITH(NOLOCK)
							    ...
							    INNER JOIN XFC_CEBES ce WITH(NOLOCK)
							        ON ds.unit_code = ce.cebe
								LEFT JOIN XFC_ComparativeCEBESAux cca WITH(NOLOCK)
									ON ds.unit_code = cca.cebe
									AND cca.year = {wfYear}
									AND cca.desc_annualcomparability IN ('Comparables','Cerradas', 'Reformas', 'Reformas Año Anterior')
							    WHERE
