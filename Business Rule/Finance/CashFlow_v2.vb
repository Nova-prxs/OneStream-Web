...
'								'Formula pass 10
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Intangibles_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
...
'								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
						...
						Case "F_Property_plant_and_equipment_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Property_plant_and_equipment_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
...
'							F_Bank_loans_and_other_loans_LT_Mov_1	F_Bank_loans_and_other_loans_LT_Mov_4	F_Bank_loans_and_other_loans_LT_Mov_5	F_Bank_loans_and_other_loans_LT_Mov_6
						Case "F_Bank_loans_and_other_loans_LT_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Bank_loans_and_other_loans_LT_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
						...
						Case "F_RoU_Mov_1"
							If BoolLocalCurrency 
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_RoU_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
...
'								AccountFilter = "A#PPAROU403"
						...
						Case "F_Equity_investments_Mov_1"
							 If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Equity_investments_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
						...
						Case "F_Deferred_tax_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Deferred_tax_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
						...
						#Region "______________Other non current non financial assets_____________"
				         Case "F_Other_non_current_non_financial_assets_Mov_1"
							 If BoolLocalCurrency
								...
								'Formula= Current month YTD-December previous closing YTD
								AccountFilter = "A#1_1_12_2.Base"'"A#671000,A#672000,A#676100,A#676200,A#677000,A#680000,A#Z67620"
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Other_non_current_non_financial_assets_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
						...
						#Region "______________Inventories______________"
			             Case "F_Inventories_Mov_1"
							If BoolLocalCurrency 
								...
								'Formula= Current month YTD-December previous closing YTD
								AccountFilter = "A#1_2_1.BASE"' "A#561000,A#560000,A#562000"
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Inventories_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
				         Case "F_Inventories_Mov_3"
							 If BoolForeignCur
								...
								'Formula= (( Balance December previous year (accounts 561000,560000,562000)/December previous year closing FX)-
								'( Balance December previous year (accounts 561000,560000,562000)/current month closing FX))'
								AccountFilter = "A#1_2_1.BASE"' "A#561000,A#560000,A#562000"
								For Each ParentMem As Member In api.Members.GetParents(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,strFlowname),False)
									If api.Members.IsDescendant(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,"Top_Constraints"), ParentMem.MemberId)
										ParentFLOW = "F#" & ParentMem.name
						...
						#Region "______________Trade Receivables______________"
						 ...
						 Case "F_Trade_receivables_Mov_1"
							 If BoolLocalCurrency 
								...
								'AccountFilter = "A#510000,A#511000,A#513000,A#515000,A#520000,A#521000,A#522000,A#123404"
								For Each ParentMem As Member In api.Members.GetParents(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,strFlowname),False)
									If api.Members.IsDescendant(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,"Top_Constraints"), ParentMem.MemberId)
										ParentFLOW = "F#" & ParentMem.name
									Continue For
...
'							End If
						...
						#Region "______________Short term interest bearing______________"
						  Case "F_Short_term_interest_bearing_advances_Mov_1"
							 If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Short_term_interest_bearing_advances_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
	...
	'							'Formula= Current month YTD-December previous closing YTD
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Cash_and_cash_equivalents_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
...
'							'Formula= Current month YTD-December previous closing YTD
						...
						#Region "______________Provision ST LT (short term Long Term)______________"
			             ...
			             Case "F_Provisions_ST_LT_Mov_1"
							 If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Provisions_ST_LT_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
						...
						#Region "______________Employee_retirement_and_post_employment_benefit_obligations______________"
					     Case "F_Employee_retirement_and_post_employment_benefit_obligations_Mov_1"
							 If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Employee_retirement_and_post_employment_benefit_obligations_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
			            ...
			            Case "F_Overdrafts_and_short_term_bank_borrowings_Mov_1"
							...
							'Formula= Current month YTD-December previous closing YTD
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Overdrafts_and_short_term_bank_borrowings_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
			    	   ...
			    	   Case "F_Other_non_current_non_financial_liabilities_Mov_1"
						   If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Other_non_current_non_financial_liabilities_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
						...
						Case "F_Trade_payables_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Trade_payables_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
					...
					#End Region
		             ...
		             Case "F_CIT_receivable_payable_Mov_1"
						If BoolLocalCurrency
							...
							Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_CIT_receivable_payable_Movements{TopPOVSource}"
							Destination = $"F#{strFlowname}{TopPOVDestination}"
							Formula = $"{Destination}=RemoveZeros({source})"
							api.Data.Calculate(Formula, accountFilter:=AccountFilter)
						End If 
						...
						#Region "______________ACCRUED INTEREST______________"
	                   	Case "F_Accrued_interest_Mov_1" 
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Accrued_interest_Movements{TopPOVSource}"
		'						Source = $"F#F_CLO:A#750500{TopPOVSource}-F#F_OPE:A#750500{TopPOVSource}-F#F_Accrued_interest_Mov_2:A#750500{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
		...
		'						AccountFilter = "A#430000"
		            	...
		            	Case "F_Accrued_interest_Leases_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Accrued_interest_Leases_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If
						...
						Case "F_Other_current_non_financial_liabilities_STIP_LTIP_Mov_1"
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_F_Other_current_non_financial_liabilities_STIP_LTIP_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
						...
						#Region "______________Deferred_consideration_ST_LT_Loand_PRP______________"
			                Case "F_Deferred_consideration_ST_LT_Loand_PRP_Mov_1"
								If BoolLocalCurrency
									...
									Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Deferred_consideration_ST_LT_Loand_PRP_Movements{TopPOVSource}"
									Destination = $"F#{strFlowname}{TopPOVDestination}"
									Formula = $"{Destination}=RemoveZeros({source})"
									api.Data.Calculate(Formula, accountFilter:=AccountFilter)
								End If 
	                    ...
	                    Case "F_Capital_Mov_1"
							...
							'Formula= Current month YTD-December previous closing YTD
							AccountFilter = "A#2_1_1_1.Base.Where(Text3 Contains CTA)"
							For Each ParentMem As Member In api.Members.GetParents(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,strFlowname),False)
								If api.Members.IsDescendant(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,"Top_Constraints"), ParentMem.MemberId)
									ParentFLOW = "F#" & ParentMem.name
									Continue For
		                ...
		                Case "F_Share_Premium_Mov_1"
							...
							'Formula= Current month YTD-December previous closing YTD
							AccountFilter = "A#2_1_1_2.Base.Where(Text3 Contains CTA)"
							For Each ParentMem As Member In api.Members.GetParents(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,strFlowname),False)
								If api.Members.IsDescendant(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,"Top_Constraints"), ParentMem.MemberId)
									ParentFLOW = "F#" & ParentMem.name
									Continue For
						...
						#Region "______________Minority interest______________"
	                  	Case "F_Minority_interest_Mov_1"
							...
							'Formula= Current month YTD-December previous closing YTD
							If BoolLocalCurrency
								AccountFilter = "A#940000, A#940002"
								For Each ParentMem As Member In api.Members.GetParents(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,strFlowname),False)
									If api.Members.IsDescendant(api.Pov.FlowDim.DimPk, api.Members.GetMemberId(dimtype.Flow.Id,"Top_Constraints"), ParentMem.MemberId)
										ParentFLOW = "F#" & ParentMem.name
										Continue For
...
'							'Formula= Current month YTD-December previous closing YTD
		                ...
		                Case "F_Retained_Earnings_Mov_1"
							...
							If BoolLocalCurrency
								...
								Formula = $"{Destination}=RemoveZeros({source})"
'								Brapi.ErrorLog.LogMessage(si,"F_Retained_Earnings_Mov_1: " & Formula)
								api.Data.Calculate(Formula)
...
'								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
	                    ...
	                    Case "F_Actuarial_valuation_Mov_1"
							...
							If BoolLocalCurrency
								...
								Source = $"F#F_CLO{TopPOVSource}-F#F_OPE{TopPOVSource}-F#F_Actuarial_valuation_Movements{TopPOVSource}"
								Destination = $"F#{strFlowname}{TopPOVDestination}"
								Formula = $"{Destination}=RemoveZeros({source})"
								api.Data.Calculate(Formula, accountFilter:=AccountFilter)
							End If 
