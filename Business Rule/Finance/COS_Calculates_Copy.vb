...
Namespace OneStream.BusinessRule.Finance.COS_Calculates_Copy
			...
			Try
				Select Case api.FunctionType					
					...
					Case Is = FinanceFunctionType.CustomCalculate			
						...
						Dim sText1 As String = BRApi.Finance.Entity.Text(si, Api.Pov.Entity.MemberId,1,0,iTime)
			...
			Or (sScenario = "Forecast" And iMonth >= iForecastMonth) Then
				...
				If (sScenario = "Budget") Then
					sCalcSameMonthAA = $"S#{sScenario}:T#{sPeriod}:E#{sEntity}:A#{sAccount}:F#Perc_Initial_Result:U1#{sChannel}:O#Forms =	
									...
									S#Forecast:T#{sActualYear}M{iMonth.toString}:E#{sEntity}:A#{sAccount}:F#None:U1#{sChannel}:O#Top
...
'				Api.Data.Calculate(sCalcAmount,True)
		...
		Sub CopyActualToForecast(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As FinanceRulesApi, ByVal args As FinanceRulesArgs, ByVal sChannel As String)
			...
			Dim sCalcDifRT As String = $"A#Dif_RT:F#None:U1#None = A#Cost_of_Sales_Contable:F#None:U1#Top - A#Cost_of_Sales_Aux:F#None:U1#Top"			
			...
			Select Case sFlow
				...
				Case "PriceIncrease"
					...
					sCalc1 = $"E#{sEntity}:A#{sAccount}:F#Perc_IncPriceExi_Input:U1#{sChannel}:O#Forms = 
							   E#{sBrand}_Input:A#{sAccount}:F#Perc_IncPriceExi_Input:U1#{sChannel}:O#Forms
					...
					sCalc2 = $"E#{sEntity}:A#{sAccount}:F#Perc_IncPriceNew_Input:U1#{sChannel}:O#Forms = 
							   E#{sBrand}_Input:A#{sAccount}:F#Perc_IncPriceNew_Input:U1#{sChannel}:O#Forms
