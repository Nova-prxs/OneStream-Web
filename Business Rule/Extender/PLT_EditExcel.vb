...
Namespace OneStream.BusinessRule.Extender.PLT_EditExcel
...
'					            cell.DataType = New EnumValue(Of CellValues)(CellValues.String)
					...
					' 🔴 PASO 2: Eliminar todas las filas a partir de startRow para evitar conflictos de orden
					Dim rowsToRemove As List(Of Row) = sheetData.Elements(Of Row)().Where(Function(r) CInt(r.RowIndex.Value) >= startRow).ToList()
					For Each rowToRemove As Row In rowsToRemove
						rowToRemove.Remove()
					Next
										
				#Region "Insertando Datos"	
				
					' Insertar los datos nuevos en orden correcto
                    For rowIndex As Integer = startRow To endRow
						Dim dtRowIndex As Integer = rowIndex - startRow
                        ' Crear la fila nueva con el índice correcto
                        Dim row As New Row() With {.RowIndex = CType(rowIndex, UInt32)}
                        ...
                        For colIndex As Integer = 1 To dt.Columns.Count
							...
							Dim colStringName() As String = {"id_product", "id_component", "id_account"}
 							Dim colName As String = dt.Columns.Item(colIndex-1).ColumnName
							
							' Crear la celda con los datos correctos
							...
							' Insertar la celda en orden alfabético dentro de la fila
							InsertCellInOrder(row, cell)
                            modified = True
                        Next
						
						' Insertar la fila completa al final del SheetData (en orden numérico de filas)
