...
'						Dim wfTime As String = BRApi.Workflow.General.GetGlobalTime(si)
							...
							For Each pksWFLoadStep As WorkflowUnitClusterPk In wfList
								...
								If Not completed Then
									' If AutoCompleteLoadAndProcessWF failed, concatenate the workflow name
										brapi.ErrorLog.LogMessage(si, "completed > " & completed.ToString)
						...
						If Not String.IsNullOrEmpty(sWFnameAll) Then
							SendErrorEmailHTML(si, args, "Error during Autoload for Workflow(s): " & sWFnameAll)
						Else
							''SendErrorEmailHTML(si, args, "Error during Autoload for Workflow(s): No Workflows were found")
						End If
'						brapi.ErrorLog.LogMessage(si, "sWFnameAll > " & sWFnameAll)
				End Select
...
'            BRApi.ErrorLog.LogMessage(si, "ERROR: Import process did not complete successfully.")
            Return False
        End If
...
'            BRApi.ErrorLog.LogMessage(si, "ERROR: Validation of transformation did not complete successfully.")
            Return False
        End If
...
'            BRApi.ErrorLog.LogMessage(si, "ERROR: Validation of intersections did not complete successfully.")
            Return False
        End If
...
'            BRApi.ErrorLog.LogMessage(si, "ERROR: Load cube process did not complete successfully.")
            Return False
        End If
...
Private Sub SendErrorEmailHTML(ByVal si As SessionInfo, ByVal args As ExtenderArgs, ByVal errorMessage As String)
    Dim toEmailAddresses As New List(Of String)({"david.gomez@aiqos.io"})
    Dim emailConnectionName As String = "OneStreamEmail"
    Dim subject As String = "Error Notification from OneStream AutoLoad"
   brapi.ErrorLog.LogMessage(si, "errorMessage >>>>>>> " & errorMessage) 
