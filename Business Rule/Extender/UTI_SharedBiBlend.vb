...
Namespace OneStream.BusinessRule.Extender.UTI_SharedBiBlend
	Public Class shared_BiBlend
	
		Public Function CopyTable(
						ByVal si As SessionInfo,
						ByVal sTable As String, _
						Optional sFactory As String = "", _
						Optional sMonth As String = "10", _
						Optional sYear As String = "2025", _
						Optional sScenario As String = "Actual"
						) As Object
            Try	
				If si.AppName <> "Production" Then
					
					brapi.ErrorLog.LogMessage(si, "Ejecutar solo en el entorno de PRODUCCIÓN")
					
					Return Nothing
					
				Else If si.AppName = "Production" Then
					Dim sFactoryFilter As String = If(sFactory="", "", $"AND id_factory='{sFactory}'")
					Dim dicTableFilter As New Dictionary(Of String, String) From {
							{ "XFC_PLT_FACT_Costs_VTU_Final"					, $"AND scenario='{sScenario}' AND YEAR(date)={sYear} {sFactoryFilter}"}, 	
							{ "XFC_PLT_FACT_Costs_VTU_Final_Local"				, $"AND scenario='{sScenario}' AND YEAR(date)={sYear} {sFactoryFilter}"},	
							{ "XFC_PLT_FACT_Costs_VTU_Report_Account"			, $"AND scenario='{sScenario}' AND YEAR(date)={sYear} {sFactoryFilter}"},	
							{ "XFC_PLT_FACT_Costs_VTU_Report_Account_Local"		, $"AND scenario='{sScenario}' AND YEAR(date)={sYear} {sFactoryFilter}"},	
							{ "XFC_PLT_HIER_Nomenclature_Date_Report"			, $"AND scenario='{sScenario}' AND YEAR(date)={sYear} {sFactoryFilter}"}	
						}
					...
					Dim sDelete As String = $"
						...
						WHERE 1=1
							{dicTableFilter(sTable)}
					"
					ExecuteSQLBIBlend(si, sDelete)
	
					BRApi.Database.SaveCustomDataTable(si, "OneStream BI Blend" , sTable, dt, True)
