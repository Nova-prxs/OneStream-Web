		...
		Public Function GetDynamicComponentsForDynamicDashboard(ByVal si As SessionInfo, ByVal api As IWsasDynamicDashboardsApiV800, ByVal workspace As DashboardWorkspace, _
		    ...
		    Try
		        If (api IsNot Nothing) Then
		            ...
		            ' BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Processing dashboard: '{dynamicDashboardEx.DynamicDashboard.Name}'")
		
		            ' Verificar si es un dashboard que necesita filtrado por seguridad
		            ' Usar Contains para ser más flexible con los nombres
		            If dynamicDashboardEx.DynamicDashboard.Name.XFEqualsIgnoreCase("EDIT_LandingPage_MainNavigation_Main_Left_Tabs") OrElse _
		               dynamicDashboardEx.DynamicDashboard.Name.Contains("_LeftTabs_Tabs") Then

		                ' BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Dashboard '{dynamicDashboardEx.DynamicDashboard.Name}' matches security filter - processing components")
...
'		                BRApi.ErrorLog.LogMessage(si, $"[LandingPage] User: '{userName}' belongs to groups: {String.Join(", ", userGroupsList)}")
...
'		                            BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Checking if user belongs to securitygroup='{securityGroup}'")
...
'		                                BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Component will be CREATED - user '{userName}' belongs to group '{securityGroup}'")
		                            Else
		                                ' NO crear el componente para este usuario
		                                shouldCreateComponent = False
'		                                BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Component will be SKIPPED - user '{userName}' does NOT belong to group '{securityGroup}'")
		                            End If
		                        Else
		                            nextLevelTemplateSubstVarsToAdd.Add(variable, wsComponent.TemplateSubstVars(variable))
		                        End If
...
'		                        BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Component ADDED to repeatArgsList")
		                    Else
'		                        BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Component NOT ADDED to repeatArgsList - will not be created")
		                    End If
...
'		            BRApi.ErrorLog.LogMessage(si, $"[LandingPage] Dashboard '{dynamicDashboardEx.DynamicDashboard.Name}' does NOT match security filter - using default processing")
