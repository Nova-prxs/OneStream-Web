...
namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Extender.extender_import_data_SQL
{
	public class MainClass
	{
			...
			try
			{
				switch (args.FunctionType)
				{
					...
					case ExtenderFunctionType.ExecuteDataMgmtBusinessRuleStep:
						...
						// ETL config
						EtlGenerationOptions etlOptions = this.GetEtlGenerationOptions2(si, tableName);
						...
						BRApi.ErrorLog.LogMessage(si, 
							$"[extender_import_data_SIC] p_table='{tableName}', p_method='{method}'");
						
						// STEP 1: Build SQL dynamically
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 1: Building SQL query...");
						
						string sqlQuery = this.BuildSqlQuery(si, tableName);
						
						// STEP 2: Execute SQL
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 2: Creating DataTable from SQL...");
						
						DataTable dt = this.CreateDataTableFromSqlQuery(si, sqlQuery);
						
						// Log resultado
						BRApi.ErrorLog.LogMessage(si, 
							$"[extender_import_data_SIC] STEP 2 DONE: DataTable rows={dt.Rows.Count}, cols={dt.Columns.Count}");
						
						//Get column mapping dictionary (after DataTable creation to allow dynamic mapping)
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 2: Getting column mapping...");
						columnMappingDict = this.GetColumnMappingDict(si, tableName, dt);
						BRApi.ErrorLog.LogMessage(si, $"[extender_import_data_SIC] STEP 2 DONE: {columnMappingDict.Count} mappings");

						//Map and filter columns in DataTable
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 3: MapAndFilterColumns...");
						dt = (DataTable)UTISharedFunctionsBR.MapAndFilterColumnsInDataTable(si, dt, columnMappingDict);
						BRApi.ErrorLog.LogMessage(si, $"[extender_import_data_SIC] STEP 3 DONE: cols={dt.Columns.Count}");
						//Create a Raw Table
						string sCreateRaw = @$"
						...
						BRApi.ErrorLog.LogMessage(si, $"[extender_import_data_SIC] STEP 5: LoadDataTableToCustomTable method='{method}'...");
						UTISharedFunctionsBR.LoadDataTableToCustomTable(si, tableNameRaw, dt, method);
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 5 DONE - Import complete");

						//Load raw into final data table
						BRApi.ErrorLog.LogMessage(si, $"[extender_import_data_SIC] STEP 6: Load raw into final data table...");
						string sETL = GenerateEtlSql(si, tableName, tableNameRaw, etlOptions);
						ExecuteSql(si, sETL);
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data_SIC] STEP 6 DONE - Import complete");
						
						//Update import tracking table
						BRApi.ErrorLog.LogMessage(si, "[extender_import_data] STEP 7: Updating import tracking table...");
						this.UpdateTableImportInfo(si, tableName);
						this.UpdateTableImportInfoSql(si, tableName);
		...
		#region "SQL functions for SIC"
		
		#region "Escenario Abierto"
		
		private string GetEscenarioAbierto(SessionInfo si)
		{
			using (DbConnInfo dbConn = BRApi.Database.CreateApplicationDbConnInfo(si))
			{
				DataTable dt = BRApi.Database.ExecuteSql(
					dbConn,
					@"SELECT TOP 1 valor 
					  FROM XFC_HR_AMD_ParamConfig WITH(NOLOCK)
				...
				{
					throw ErrorHandler.LogWrite(si, new XFException("No value found for 'escenario_abierto'."));
				}
		
				return dt.Rows[0]["valor"].ToString().Trim();
			}
		}

		#endregion
		
		#region "Periodo en cada tabla"

		private string GetPeriodColumn(SessionInfo si, string tableName)
		{
		    using (DbConnInfo dbConn = BRApi.Database.CreateApplicationDbConnInfo(si))
		    {
		        ...
		        DataTable dt = BRApi.Database.ExecuteSql(dbConn,
		            @"
		            SELECT SqlPeriodColumn
		            FROM XFC_HR_ETL_TableConfig
		        ...
		        if (dt == null || dt.Rows.Count == 0)
		            throw new XFException($"No configuration found in XFC_HR_ETL_TableConfig for table '{tableName}'.");
		
		        string periodColumn = dt.Rows[0]["SqlPeriodColumn"]?.ToString();
		
		        if (string.IsNullOrWhiteSpace(periodColumn))
		            throw new XFException($"SqlPeriodColumn is empty in XFC_HR_ETL_TableConfig for table '{tableName}'.");
		...
		private string BuildSqlQuery(SessionInfo si, string tableName)
		{
			string escenarioAbierto = this.GetEscenarioAbierto(si);
			string periodoColumn = this.GetPeriodColumn(si, tableName);
		
			string sql = $@"
				SELECT *
				FROM {tableName}
				WHERE {periodoColumn} = '{escenarioAbierto}'
			";
		...
		#region "Create DataTable From SQL Query"
			
		private DataTable CreateDataTableFromSqlQuery(SessionInfo si, string sqlQuery)
		{
			if (string.IsNullOrWhiteSpace(sqlQuery))
			{
				throw ErrorHandler.LogWrite(si, new XFException("SQL query is empty."));
			}
		
			BRApi.ErrorLog.LogMessage(si, "Executing SQL on SIC...");
		
			DataTable dt;
		
			using (DbConnInfo dbConn = BRApi.Database.CreateDbConnInfo(si, DbLocation.External, "SICPreSQL"))
			{
				dt = BRApi.Database.ExecuteSql(dbConn, sqlQuery, true);
			}
		
			if (dt == null)
			{
				throw ErrorHandler.LogWrite(si, new XFException("SQL query returned null DataTable."));
			}
		
			BRApi.ErrorLog.LogMessage(si, 
				$"SQL result rows={dt.Rows.Count}, cols={dt.Columns.Count}");
		...
		private Dictionary<string, TableEtlConfiguration> GetTableEtlConfigurations()
		{
		    ...
		    var defaultConfig = new TableEtlConfiguration
		    {
		        ...
		        SqlDefaultDateInputFormats = new List<string> { "auto" },
		    ...
		    return new Dictionary<string, TableEtlConfiguration>(StringComparer.OrdinalIgnoreCase)
		    {
		        ...
		        ["XFC_HR_Helper"] = new TableEtlConfiguration
		        {
		            ...
		            SqlDefaultDateInputFormats = new List<string> { "auto", "dd/mm/yyyy", "yyyy-mm-dd" },
		        ...
		        ["XFC_HR_MASTER_GLB_Empleados"] = new TableEtlConfiguration
		        {
		            ...
		            FloatColumnsNumericFormat = new NumericFormatConfiguration { DecimalSeparator = ",", ThousandsSeparator = "." },
		            SqlDefaultDateInputFormats = new List<string> { "dd.mm.yyyy" },
		            DateColumnInputFormats = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase)
		            {
		                ["periodo"] = new List<string> { "yyyymm" }
		            },
...
//		}
		...
		private List<string> ResolveDateFormats(string columnName, EtlGenerationOptions options)
		{
			...
			if (options.SqlDefaultDateInputFormats.Count > 0)
			{
				return options.SqlDefaultDateInputFormats;
			}
...
//		        $"FileDateFormats=[{fileDates}]; " +
		...
		/// <summary>
		/// Upserts a row in XFC_HR_MASTER_TableImportInfo to track the last import operation via file.
		/// </summary>
		private void UpdateTableImportInfo(SessionInfo si, string tableName)
		{
		    using (DbConnInfo dbConn = BRApi.Database.CreateApplicationDbConnInfo(si))
		    {
		        var dbParamInfos = new List<DbParamInfo>
		        ...
		        BRApi.Database.ExecuteSql(
		            ...
		            WHEN NOT MATCHED THEN
		                INSERT (table_name, username, last_import_date, import_type, period)
		                VALUES (@paramTableName, @paramUserName, GETDATE(), @paramImportType, NULL);
		            ",
		            dbParamInfos,
		            false
		        );
		...
		/// <summary>
		/// Upserts a row in XFC_HR_MASTER_TableImportInfo to track the last import operation via external SQL source.
		/// Period is retrieved from XFC_HR_AMD_ParamConfig (escenario_abierto).
		/// </summary>
		private void UpdateTableImportInfoSql(SessionInfo si, string tableName)
		{
		    using (DbConnInfo dbConn = BRApi.Database.CreateApplicationDbConnInfo(si))
		    {
		        var dbParamInfos = new List<DbParamInfo>
		        {
		            new DbParamInfo("paramTableName", tableName),
		            new DbParamInfo("paramUserName", si.UserName),
		            new DbParamInfo("paramImportType", "sql")
		        };
		        BRApi.Database.ExecuteSql(
		            dbConn,
		            @"
		            DECLARE @paramPeriod NVARCHAR(100) = (
		                SELECT valor 
		                FROM XFC_HR_AMD_ParamConfig 
		                WHERE parametro = 'escenario_abierto'
		            );
		            ...
		            WHEN NOT MATCHED THEN
		                INSERT (table_name, username, last_import_date, import_type, period)
		                VALUES (@paramTableName, @paramUserName,GETDATE(), @paramImportType, @paramPeriod);
		            ",
		            dbParamInfos,
		            false
		        );
