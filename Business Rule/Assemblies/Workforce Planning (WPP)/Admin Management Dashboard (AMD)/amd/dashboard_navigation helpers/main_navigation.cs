				...
				switch (args.FunctionType)
				{
					...
					case DashboardExtenderFunctionType.LoadDashboard:
						if (args.FunctionName.XFEqualsIgnoreCase("LoadDashboard"))
						{
							...
							string WorkspacePrefix = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, false, Guid.Empty, "WPP.prm_amd_Workspace_Prefix");
					...
					case DashboardExtenderFunctionType.ComponentSelectionChanged:
						...
						#region "Set Table Import Params"

						if (args.FunctionName.XFEqualsIgnoreCase("SetTableImportParams"))
						{
							// Persist dialog parameter values so |!...!| resolves inside the DynamicGrid
							string[] paramKeys = new string[]
							{
								"prm_amd_Component_TableImport_TableName",
								"prm_amd_Component_TableImport_TableFilters",
								"prm_amd_Component_TableImport_PublicName",
								"prm_amd_Component_TableImport_AditionalInfo"
							};
							string[] nvpKeys = new string[] { "p_table_name", "p_table_filters", "p_public_name", "p_aditional_info" };

							for (int i = 0; i < paramKeys.Length; i++)
							{
								string val = string.Empty;
								if (args.NameValuePairs != null && args.NameValuePairs.ContainsKey(nvpKeys[i]))
									val = args.NameValuePairs[nvpKeys[i]] ?? string.Empty;
								BRApi.Dashboards.Parameters.SetLiteralParameterValue(si, false, paramKeys[i], val);
							}
