			...
			Try
				Select Case args.FunctionType
					...
					Case Is = DashboardDataSetFunctionType.GetDataSetNames
						...
						names.Add("FxRates")
						names.Add("ComparisonActualCashBySite")
						Return names
					...
					Case Is = DashboardDataSetFunctionType.GetDataSet
						...
						#Region "Comparison Cash Actual Graph"
							...
							ElseIf args.DataSetName.XFEqualsIgnoreCase("ComparisonActualCash") Then
							    ...
							    Dim scenarioGraph As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "Investments.prm_Investments_Cash_Scenario_Graph")
						...
						#Region "Comparison Cash Actual Graph By Site"
						
						ElseIf args.DataSetName.XFEqualsIgnoreCase("ComparisonActualCashBySite") Then
						    'Retrieve parameters for queries
						    Dim queryParams As String = args.NameValuePairs("p_query_params")
						    Dim parameterDict As Dictionary(Of String, String) = UTISharedFunctionsBR.SplitQueryParams(si, queryParams)
						    Dim dbParamInfos As List(Of DbParamInfo) = UTISharedFunctionsBR.CreateQueryParams(si, queryParams)
						    ...
						    Dim dt1Site As New DataTable()
						    Dim dt2Site As New DataTable()
						
						    'Build the whereClause for the FIRST query (Cash_Cumul_Graph).
						    'The original code already includes v.year = @paramYear AND v.scenario = 'Actual' or @paramScenario,
						    'so we only append paramType, paramCompany, and paramSite if applicable:
						    Dim whereClauseSite As String = ""
						    If Not parameterDict("paramType").Equals("(Any)") Then
						        whereClauseSite += "AND m.type = @paramType "
						    End If
						
						    If Not parameterDict("paramCompany").Equals("0") Then
						        whereClauseSite += "AND m.company_id = @paramCompany "
						    End If
							
							If Not parameterDict("paramSite").Equals("(Any)") Then
							    whereClauseSite += "AND m.site = @paramSite "
							End If
						
						    '-- FIRST QUERY Actual | paraScenario
						    Using dbConnSite As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
						        'Construct and execute the query for cumulative cash data
						        Dim selectQuerySite As String = $"
						            WITH Cash_Cumul_Graph AS (    
						                SELECT
						                    'Actual' AS Scenario,
						                    SUM(cash_m1)  AS Jan,
						        ...
						        'Execute the query and populate the DataTable
						        dtSite = BRApi.Database.ExecuteSql(dbConnSite, selectQuerySite, dbParamInfos, False)
						    End Using
						
						    ' Build the whereClause for the SECOND query
						    Dim whereClause2Site As String = "f.year = @paramYear AND f.scenario = 'Actual' "
						    
						    If Not parameterDict("paramType").Equals("(Any)") Then
						        whereClause2Site += "AND m.type = @paramType "
						    End If
						    
						    If Not parameterDict("paramCompany").Equals("0") Then
						        whereClause2Site += "AND m.company_id = @paramCompany "
						    End If
						    
						    If Not parameterDict("paramSite").Equals("(Any)") Then
						        whereClause2Site += "AND m.site = @paramSite "
						    End If
						
						    ' SECOND QUERY: Monthly Cash Data
						    Using dbConn1Site As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
						        Dim selectQuery1Site As String = $"
						            WITH monthlyCash AS (
						                SELECT
						                    SUM(CASE WHEN f.month = 1  THEN f.amount ELSE 0 END) AS Jan,
						                    SUM(CASE WHEN f.month = 2  THEN f.amount ELSE 0 END) AS Feb,
						        ...
						        ' Execute the first query and populate the DataTable
						        dt1Site = BRApi.Database.ExecuteSql(dbConn1Site, selectQuery1Site, dbParamInfos, False)
						    End Using
						
						    'THIRD QUERY: Budget Cumulative Cash Data
						    Using dbConn2Site As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
						        Dim selectQuery2Site As String = $"
						            WITH Cash_Cumul_Graph AS (
						                SELECT
						                    SUM(cash_m1)  AS Jan,
						                    SUM(cash_m2)  AS Feb,
						        ...
						        ' Execute the second query and populate the DataTable
						        dt2Site = BRApi.Database.ExecuteSql(dbConn2Site, selectQuery2Site, dbParamInfos, False)
						    End Using
						
						    ' -- Construction of the series collection --------------------------------
						    Dim oSeriesCollectionSite As New XFSeriesCollection()
						    
						    ' Check if the table contains data for the "Actual" scenario
						    Dim dtActualSite As DataTable
						    If dtSite.Select("Scenario = 'Actual'").Length > 0 Then
						        dtActualSite = dtSite.Select("Scenario = 'Actual'").CopyToDataTable()
						    Else
						        dtActualSite = dtSite.Clone() ' Creates an empty table with the same structure
						    End If
						    
						    ' Check if the table contains data for the scenario specified by the parameter
						    Dim dtScenarioSite As DataTable
						    If dtSite.Select($"Scenario = '{parameterDict("paramScenario")}'").Length > 0 Then
						        dtScenarioSite = dtSite.Select($"Scenario = '{parameterDict("paramScenario")}'").CopyToDataTable()
						    Else
						        dtScenarioSite = dtSite.Clone() ' Creates an empty table with the same structure
						    End If
						    
						    ' Add series for the "Actual Cash Out" scenario
						    oSeriesCollectionSite.AddSeries(CreateSeries(si, "Actual Cash Out", XFChart2SeriesType.BarSideBySide, primaryColor, dtActualSite, "Amount", "Month", False, False, 0, 0, XFChart2ModelType.Bar2DBorderlessSimple, 0.8))
						    
						    ' Add series for the parameter-dependent scenario
						    oSeriesCollectionSite.AddSeries(CreateSeries(si, $"{parameterDict("paramScenario")}", XFChart2SeriesType.Line, secondaryColor, dtScenarioSite, "Amount", "Month", False, False, 12, 4, XFChart2ModelType.Bar2DBorderlessSimple, 0.8))
						    
						    ' Add series for "Monthly Cash"
						    oSeriesCollectionSite.AddSeries(CreateSeries(si, "Monthly Cash", XFChart2SeriesType.Line, terciaryColor, dt1Site, "Amount", "Month", True, True, 1, 4, , ))
						    
						    ' Add series for "Budget"
						    oSeriesCollectionSite.AddSeries(CreateSeries(si, "Budget", XFChart2SeriesType.Line, "Green", dt2Site, "Amount", "Month", True, True, 1, 4, , ))
						    
						    ' Return the constructed dataset
						    Return oSeriesCollectionSite.CreateDataSet(si)
						...
						ElseIf args.DataSetName.XFEqualsIgnoreCase("RND_Actual_Budget_Comparison_Graph") Then
						    ...
						    Dim secondaryColor As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "Shared.prm_Style_Colors_Secondary")
						    Dim terciaryColor As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "Shared.prm_Style_Colors_Terciary")
						    Dim scenarioComparison As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "Investments.prm_Investments_Cash_Scenario_Graph")
			...
			For i As Integer = 1 To 12
				newRow($"M{i}") = Decimal.Round(
					BRApi.Finance.Data.GetDataCellUsingMemberScript(si,
						"Horse",
						$"E#{companyMemberName}:P#{parentMember}:C#{consolidationMember}:S#Actual:T#{parameterDict("paramYear")}M{i}:V#Periodic:A#TINV023:F#None:O#Top:I#Top:U1#Top:U2#Top:U3#Top:U4#CF_AuditTrail:U5#None:U6#None:U7#Top:U8#None"
