        ...
        Public Function GetMigrationQuery(ByVal si As SessionInfo, ByVal type As String) As String
            Try
				...
				BEGIN
					...
					CREATE INDEX idx_company_id ON XFC_INV_MASTER_project (company_id);
					CREATE INDEX idx_type ON XFC_INV_MASTER_project (type);
					CREATE INDEX idx_aggregate ON XFC_INV_MASTER_project (aggregate);
					CREATE INDEX idx_project_company_id ON XFC_INV_MASTER_project (project, company_id);
				...
				Dim upQuery As String = $"
					...
					USING (
					    SELECT 
							...
							ma_date,
							total_requirement
					    FROM XFC_INV_RAW_project p
						INNER JOIN XFC_INV_MASTER_company c ON p.company_id = c.id
					) AS source
					ON target.project = source.project
					WHEN MATCHED THEN
					    UPDATE SET
					        func = source.func,
							project_id_new = source.project_id_new,
					        ...
					        type = CASE
								WHEN target.type IS NOT NULL
									AND target.type NOT IN ('', 'No type') THEN target.type
								ELSE source.type
							END,
					        ...
					        aggregate = CASE
								WHEN target.aggregate IS NOT NULL
									AND target.aggregate NOT IN ('', 'No aggregate') THEN target.aggregate
					        ...
					        budget_owner = CASE
								WHEN target.budget_owner IS NOT NULL AND target.budget_owner <> '' THEN target.budget_owner
								ELSE source.budget_owner
							END,
					        ma_date = CASE
								WHEN target.ma_date IS NOT NULL AND YEAR(target.ma_date) > 2000 THEN target.ma_date
								ELSE source.ma_date
							END,
							is_real = 1,
							is_closed = 0,
...
'		ALTER TABLE XFC_INV_MASTER_project
'		ADD is_closed BIT DEFAULT 0;

'----------------------------------------------------------------------

'		UPDATE XFC_INV_MASTER_project
'		SET is_closed = 0;
...
'		SET is_closed = 1,
'			is_real = 0
'		WHERE project LIKE 'ASSET%';

'----------------------------------------------------------------------
'		CREATE INDEX idx_project ON XFC_INV_MASTER_project (project);
'		CREATE INDEX idx_company_id ON XFC_INV_MASTER_project (company_id);
'		CREATE INDEX idx_type ON XFC_INV_MASTER_project (type);
'		CREATE INDEX idx_aggregate ON XFC_INV_MASTER_project (aggregate);
'		CREATE INDEX idx_project_company_id ON XFC_INV_MASTER_project (project, company_id);
