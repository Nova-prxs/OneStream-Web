				...
				Dim upQuery As String = $"
					...
					BEGIN
						CREATE TABLE {tableName} (
							...
							CONSTRAINT PK_{tableName} PRIMARY KEY (id, project_id)
						);
				
						CREATE INDEX idx_project_id ON XFC_INV_MASTER_asset (project_id);
						CREATE INDEX idx_asset_id ON XFC_INV_MASTER_asset (id);
						CREATE INDEX idx_asset_id_project_id ON XFC_INV_MASTER_asset (id, project_id);
...
'		Where type = 'PK' AND parent_object_id = OBJECT_ID('XFC_INV_MASTER_asset');
		
'		-- Construir la cadena de comando dinámico
'		Set @sql = 'ALTER TABLE XFC_INV_MASTER_asset DROP CONSTRAINT ' + QUOTENAME(@constraintName);
		
'		-- Ejecutar la cadena de comando
'		EXECUTE sp_executesql @sql;

'		ALTER TABLE XFC_INV_MASTER_asset
'		ADD CONSTRAINT PK_XFC_INV_MASTER_asset
