			    ...
			    IF OBJECT_ID(N'{viewName}', N'V') IS NULL
			        BEGIN
			            EXEC('CREATE VIEW {viewName} AS
			                SELECT
			                        pma.*,
			                        p.project_id_new AS project_id_new,
			                        CASE
			                        ...
			                        p.site AS site, -- <<< COLUMNA SITE AÑADIDA >>>
			                    ...
			                    FROM (
			                        SELECT 
			                            ...
			                            scenario,
			                            CASE
			                                WHEN scenario LIKE ''RF%'' AND ISNUMERIC(REPLACE(scenario, ''RF'', '''')) = 1 
			                                THEN CAST(REPLACE(scenario, ''RF'', '''') AS INTEGER)
			                    ...
			                    LEFT JOIN (
			                        ...
			                        FROM (
			                            ...
			                            FROM XFC_INV_FACT_project_cash
			                            WHERE scenario LIKE ''RF%'' AND ISNUMERIC(REPLACE(scenario, ''RF'', '''')) = 1
			                        ) AS subquery
			                        WHERE rn = 1
			                    ) rfy ON
