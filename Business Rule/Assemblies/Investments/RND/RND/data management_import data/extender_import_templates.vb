...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Extender.extender_import_templates
	Public Class MainClass
			...
			Try
				Select Case args.FunctionType
					...
					Case Is = ExtenderFunctionType.ExecuteDataMgmtBusinessRuleStep
						...
						If paramFunction = "ImportRnDProjects" Then
							'Get parameters
							...
							Dim templateFileName As String = If(parameterDict("paramLoadType") = "revenue", "RnDProjectsRevenue", "RnDProjects")
							...
							For Each fileName As NameAndAccessLevel In fileNames
								...
								If Not fileName.Name.Contains(templateFileName) Then
									'Clear folder
									UTISharedFunctionsBR.DeleteAllFilesInFolder(si, sourcePath, FileSystemLocation.ApplicationDatabase)
									Throw ErrorHandler.LogWrite(si, New XFException($"Uploaded file is not correct. Check that the name begins with '{templateFileName}'."))
								End If
							...
							Me.RunPopulationQueries(si, $"XFC_RND_RAW_project_{parameterDict("paramLoadType")}")
							
						#End Region
						
						#Region "Import RnD Projects Cash Out"
						
						Else If paramFunction = "ImportRnDProjectsCashOut" Then
							'Get parameters
						    Dim queryParams As String = args.NameValuePairs("p_query_params")
						    Dim parameterDict As Dictionary(Of String, String) = UTISharedFunctionsBR.SplitQueryParams(si, queryParams)
						    Dim dbParamInfos As List(Of DbParamInfo) = UTISharedFunctionsBR.CreateQueryParams(si, queryParams)
							Dim templateFileName As String = "RnDProjectsCashOut"
							
							'Get file names in the imported data capital folder and build path to save the processed files
							Dim sourcePath As String = "Documents/Public/Investments/Imported Data/RnD/Cash Out"
							Dim targetFinalPath As String = "Processed"
							Dim targetPath As String = $"{sourcePath}/{targetFinalPath}"
							Dim fileNames As List(Of NameAndAccessLevel) = BRApi.FileSystem.GetAllFileNames(
								si,
							...
							'Run population queries for rnd raw project table
							Me.RunPopulationQueries(si, $"XFC_RND_RAW_project_cash_out")
							
						#End Region
						
						#Region "Import RnD Projects Capitalization"
						
						Else If paramFunction = "ImportRnDProjectsCapitalization" Then
							'Get parameters
						    Dim queryParams As String = args.NameValuePairs("p_query_params")
						    Dim parameterDict As Dictionary(Of String, String) = UTISharedFunctionsBR.SplitQueryParams(si, queryParams)
						    Dim dbParamInfos As List(Of DbParamInfo) = UTISharedFunctionsBR.CreateQueryParams(si, queryParams)
							Dim templateFileName As String = "RnDProjectsCapitalization"
							
							'Get file names in the imported data capital folder and build path to save the processed files
							Dim sourcePath As String = "Documents/Public/Investments/Imported Data/RnD/Capitalization"
							Dim targetFinalPath As String = "Processed"
							Dim targetPath As String = $"{sourcePath}/{targetFinalPath}"
							Dim fileNames As List(Of NameAndAccessLevel) = BRApi.FileSystem.GetAllFileNames(
								si,
							...
							'Run population queries for rnd raw project table
							Me.RunPopulationQueries(si, $"XFC_RND_RAW_project_capitalization")
		...
		Private Sub RunPopulationQueries(ByVal si As SessionInfo, ByVal tableName As String)
			...
			'Control table name
			If tableName = "XFC_RND_RAW_project_expense" Then
			...
			Else If tableName = "XFC_RND_RAW_project_revenue" Then
				tableList.Add(New Workspace.__WsNamespacePrefix.__WsAssemblyName.rnd_project_revenue)
			Else If tableName = "XFC_RND_RAW_project_cash_out" Then
				tableList.Add(New Workspace.__WsNamespacePrefix.__WsAssemblyName.rnd_project_cash_out)
			Else If tableName = "XFC_RND_RAW_project_capitalization" Then
				tableList.Add(New Workspace.__WsNamespacePrefix.__WsAssemblyName.rnd_project_capitalization)
			End If
