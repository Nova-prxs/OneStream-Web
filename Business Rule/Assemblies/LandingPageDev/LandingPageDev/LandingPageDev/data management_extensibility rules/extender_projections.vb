						...
						Else If functionName = "ProjectDnA" Then
							...
							Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
								Dim projectionQuery As String = $"
									...
									year_cte AS (
										...
										SELECT
										    ...
										    (
										        ...
										        FROM XFC_INV_AUX_date_dimension
										        WHERE
										            date BETWEEN
										...
										FROM
										    year_cte ycte
										CROSS JOIN (
										    SELECT DISTINCT a.id AS asset_id, a.initial_value, a.activation_date AS ma_date, ac.years_amortization_conso
										    FROM XFC_INV_MASTER_project p WITH(NOLOCK)
									...
									USING (
										SELECT asset_id, @paramScenario AS scenario, year, month, 'conso' AS Type, SUM(monthly_depreciation) AS amount
										FROM asset_year_month_cte
										GROUP BY asset_id, year, month
									) AS source
