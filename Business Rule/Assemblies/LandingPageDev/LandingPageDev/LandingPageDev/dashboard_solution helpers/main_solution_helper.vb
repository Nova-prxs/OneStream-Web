				...
				Select Case args.FunctionType
					...
					Case Is = DashboardExtenderFunctionType.ComponentSelectionChanged
						...
						Else If args.FunctionName.XFEqualsIgnoreCase("DownloadCashTemplate") Then
							...
							Dim sourcePath As String = "Documents/Public/LandingPageDev/Templates"
								...
								Try
									Dim insertQuery As String = $"
										...
										SELECT
											...
											0 AS is_real,
										    @paramCostCenter AS cost_center,
											@paramProject AS project_id,
											@paramDesignation AS designation,
											@paramCategory AS category_id,
											@paramMADate AS activation_date,
										    @paramInitialValue AS initial_value;
									"
									BRApi.Database.ExecuteSql(dbConn, insertQuery, dbParamInfos, False)
								Catch ex As Exception
									dbConn.RollbackTrans()
						...
						Else If args.FunctionName.XFEqualsIgnoreCase("CheckDeleteAsset") Then
							...
							'Get parameters
							Dim paramAsset As String = args.NameValuePairs("p_asset")
							...
							'Control if user has selected a project
							If String.IsNullOrEmpty(paramAsset) Then
								selectionChangedTaskResult.ChangeSelectionChangedUIActionInDashboard = True
								selectionChangedTaskResult.IsOK = False
								selectionChangedTaskResult.ShowMessageBox = True
								selectionChangedTaskResult.Message = $"You must select an asset."
								Return selectionChangedTaskResult
							End If
							
							'Control if project begins with 'ONE'
							If Not paramAsset.StartsWith("A") Then
								selectionChangedTaskResult.ChangeSelectionChangedUIActionInDashboard = True
								selectionChangedTaskResult.IsOK = False
								selectionChangedTaskResult.ShowMessageBox = True
								selectionChangedTaskResult.Message = $"You can't delete asset: {paramAsset}." & vbCrlf &
									"Asset must have been created in OneStream."
								Return selectionChangedTaskResult
							End If
						...
						Else If args.FunctionName.XFEqualsIgnoreCase("DownloadAssetTemplate") Then
							...
							Dim targetFinalPath As String = $"Import Templates"
							Dim targetPath As String = $"{targetInitialPath}/{targetFinalPath}"
							Dim targetFile As String = $"Asset{templateScenario}_{paramCompany}.xlsx"
						...
						Else If args.FunctionName.XFEqualsIgnoreCase("DeleteAsset") Then
							...
							If Not paramAsset.StartsWith("A") Then
								...
								selectionChangedTaskResult.ShowMessageBox = True
								selectionChangedTaskResult.Message = $"You can't delete asset: {paramAsset}." & vbCrlf &
									"Asset must be created in OneStream"
								Return selectionChangedTaskResult
							End If
							...
							Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
								...
								'Declare db param infos
								Dim dbParamInfos As New List(Of DbParamInfo) From {
									New DbParamInfo("paramAsset", paramAsset)
								}
								Try
									Dim deleteQuery As String = $"
										DELETE FROM XFC_INV_MASTER_asset
						...
						#Region "Refresh Aggregation Level"
						
						Else If args.FunctionName.XFEqualsIgnoreCase("RefreshAggregationLevel") Then
							
							Dim selectionChangedTaskResult As New XFSelectionChangedTaskResult()
							
							'Implement Dashboard Component Selection Changed logic here.
							Dim paramEntity As String = args.NameValuePairs.XFGetValue("p_entity")
							Dim aggregationValue As String = "C#Top:U4#Stand_Alone"
							
							If paramEntity.Contains("Horse_Group") Then
								aggregationValue = "C#Local:U4#Top"
							Else If paramEntity.Length = 5 Then
								aggregationValue = "C#Local:U4#Stand_Alone"
							End If
							selectionChangedTaskResult.IsOK = True
							selectionChangedTaskResult.ShowMessageBox = False
							...
							selectionChangedTaskResult.ModifiedCustomSubstVars.Add("LP_param_POV_Aggregation_Level", aggregationValue)
							Return selectionChangedTaskResult
