	...
	Public Class MainClass
				...
				Select Case args.FunctionType
					...
					Case Is = DashboardExtenderFunctionType.LoadDashboard
						If args.FunctionName.XFEqualsIgnoreCase("LoadDashboard") Then
							...
							Dim WorkspacePrefix As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "LPW.prm_Workspace_Prefix")
		...
		Private Sub SetDashboardParameters(
			...
			If level = 1 Then
				...
				Dim defaultForecast As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "prm_LPW_Scenario_DefaultForecast")
				Dim defaultBudget As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, "prm_LPW_Scenario_DefaultBudget")
				...
				If availableCompanies.Contains(0) OrElse availableCompanies.Count = 0 Then
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_DefaultCompany") = "0"
				Else If availableCompanies.Count > 0
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_DefaultCompany") = availableCompanies(0).ToString
				End If
				'Set scenario based on the previous selected dashboard
				If activeDashboardNumber = 3 Then
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Main") = "Planning"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Planning") = "Forecast"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Navigation_Header_Right") = "LPW_Forecast_Navigation_Header_Right"
					If Not args.LoadDashboardTaskInfo.CustomSubstVarsFromPriorRun.XFGetValue($"prm_LPW_Scenario").StartsWith("RF") Then _
						loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario") = defaultForecast
				Else If activeDashboardNumber = 4 Then
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Main") = "Planning"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Planning") = "Budget"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Navigation_Header_Right") = "LPW_Budget_Navigation_Header_Right"
					If Not args.LoadDashboardTaskInfo.CustomSubstVarsFromPriorRun.XFGetValue($"prm_LPW_Scenario").StartsWith("Budget") Then _
						loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario") = defaultBudget
				Else
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Main") = "Actual"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario_Planning") = "Actual"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario") = "Actual"
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Navigation_Header_Right") = "LPW_Actual_Navigation_Header_Right"
				End If
			...
			Else If level = 2 Then
				...
				Dim paramYear As String = If(
					loadDashboardTaskResult.ModifiedCustomSubstVars.ContainsKey($"prm_LPW_Year"),
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Year"),
					args.LoadDashboardTaskInfo.CustomSubstVarsFromPriorRun.XFGetValue($"prm_LPW_Year")
				)
				If String.IsNullOrEmpty(paramYear) Then paramYear = "2024"
				'Get scenario
				Dim paramScenario As String = If(
					loadDashboardTaskResult.ModifiedCustomSubstVars.ContainsKey($"prm_LPW_Scenario"),
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_Scenario"),
					args.LoadDashboardTaskInfo.CustomSubstVarsFromPriorRun.XFGetValue($"prm_LPW_Scenario")
				)
				...
				If paramCompany = 0 OrElse isScenarioYearConfirmed Then
					For i As Integer = 1 To 12
						loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}") = "False"
						loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}_Color") = NotEditableColor
					Next
					'Define last rf. Only select last rf if we are in budget capital or cash.
					loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_LastRF") = If(
				...
				Else
					...
					If paramScenario = "Actual" Then
							...
							If dt.Rows.Count < 1 Then
								For i As Integer = 1 To 12
									loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}") = "True"
									loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}_Color") = editableColor
								Next
							Else
								For i As Integer = 1 To 12
									If dt.Rows(0)($"M{i}").ToString = "True" Then
										loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}") = "False"
										loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}_Color") = confirmedColor
									Else
										loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}") = "True"
										loadDashboardTaskResult.ModifiedCustomSubstVars($"prm_LPW_IsEditable_{i}_Color") = editableColor
									End If
