...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.DashboardExtender.main_navigation
	Public Class MainClass
				...
				Select Case args.FunctionType
					...
					Case Is = DashboardExtenderFunctionType.LoadDashboard
						If args.FunctionName.XFEqualsIgnoreCase("LoadDashboard") Then
							...
							Dim WorkspacePrefix As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, Nothing, "ETL_Load_Control.prm_ETL_Load_Control_Workspace_Prefix")
							...
							If args.LoadDashboardTaskInfo.Reason = LoadDashboardReasonType.Initialize _
								...
								paramDashboard = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, Nothing, $"ETL_Load_Control.prm_{WorkspacePrefix}_MainNavigation_Default")
							Else
						...
						'Start from the first numeric part (skip workspace prefix parts like "ETL", "Load", "Control")
						Dim startIndex As Integer = dashboardSplitted.Length - dashboardSplitted.Where(Function(part) IsNumeric(part)).Count()
						Dim level As Integer = 1
						For i As Integer = startIndex To dashboardSplitted.Count - 1
							Dim activeDashboardNumber As Integer = CInt(dashboardSplitted(i))
		...
		Private Sub SetActiveTab(
			...
			Dim level2InActive As String = BRApi.Dashboards.Parameters.GetLiteralParameterValue(si, False, Nothing, "Shared.prm_Style_Colors_LeftBar_Tab2_InActive")
			...
			Dim colorDictionary As New Dictionary(Of Integer, Tuple(Of String, String)) From {
				...
				{2, New Tuple(Of String, String)(level2Active, level2InActive)},
...
'					loadDashboardTaskResult.ModifiedCustomSubstVars("prm_ETL_Load_Control_Scenario_Main") = "Planning"
'					If args.LoadDashboardTaskInfo.CustomSubstVarsFromPriorRun.XFGetValue("prm_ETL_Load_Control_Scenario") = "Actual" Then _
'						loadDashboardTaskResult.ModifiedCustomSubstVars("prm_ETL_Load_Control_Scenario") = "Budget"
'				Else
'					loadDashboardTaskResult.ModifiedCustomSubstVars("prm_ETL_Load_Control_Scenario_Main") = "Actual"
'					loadDashboardTaskResult.ModifiedCustomSubstVars("prm_ETL_Load_Control_Scenario") = "Actual"
'				End If
'			End If
		...
		Private Sub SetDashboardParametersOnClick(
			...
			'Control dashboard parameters
			'EXAMPLE
'			If paramDashboard = "ETL_Load_Control_1_1" Then _
'				selectionChangedTaskResult.ModifiedCustomSubstVars("prm_ETL_Load_Control_Request_Id") = ""
