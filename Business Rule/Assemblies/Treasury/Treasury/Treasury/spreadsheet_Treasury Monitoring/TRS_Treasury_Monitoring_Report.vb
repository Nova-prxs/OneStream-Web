...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Spreadsheet.TRS_Treasury_Monitoring_Report
	Public Class MainClass
		
		' Get Treasury Monitoring data - Shows ALL entities (no entity filter)
		Private Function GetTreasuryMonitoringData(ByVal si As SessionInfo, ByVal globals As BRGlobals, ByVal api As Object, ByVal args As SpreadsheetArgs) As TableView
			Try
				...
				' Build SQL query to retrieve ALL entities for the given week and year
				Dim whereClause As String = $"WHERE week_starting = {weekStarting} AND Year = {year}"
				Dim sql As String = $"
					SELECT 
						Entity_Id,
						Timekey,
			...
			' Add columns for the table (reordered)
			...
			tv.Columns.Add(New TableViewColumn() With {.Name = "AlertLevelStatus", .Value = "", .IsHeader = True, .DataType = XFDataType.Text})
			...
			tv.Columns.Add(New TableViewColumn() With {.Name = "AlertLevel", .Value = "", .IsHeader = True, .DataType = XFDataType.Int16})
					...
					' Add title row with "OVERALL treasury ANALYSIS"
					Dim titleRow As New TableViewRow() With {.IsHeader = True}
					titleRow.Items.Add("AsOf", New TableViewColumn() With {.Value = "OVERALL treasury ANALYSIS", .IsHeader = True})
					titleRow.Items.Add("BusinessUnit", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("AlertLevelStatus", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("Issue", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("WeekStarting", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("Analysis", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("Solution", New TableViewColumn() With {.Value = "", .IsHeader = True})
					titleRow.Items.Add("AlertLevel", New TableViewColumn() With {.Value = "", .IsHeader = True})
					tv.Rows.Add(titleRow)
					
					' Add header row with column titles (reordered)
					...
					headerRow.Items.Add("AlertLevelStatus", New TableViewColumn() With {.Value = "Alert Level Status", .IsHeader = True})
					...
					For Each dataRow As DataRow In dt.Rows
						...
						' Alert Level Status column - shows alert_level (●)
						tvRow.Items.Add("AlertLevelStatus", New TableViewColumn() With {
						...
						' Alert Level column - shows alert_level_status (INT)
				...
				Select Case args.FunctionType
					...
					Case Is = SpreadsheetFunctionType.GetCustomSubstVarsInUse
						Try
							...
							' No entity filter needed - report shows ALL entities
