...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Extender.TRS_ReportGenerator_CashDebt_HTD
    Public Class MainClass
        
        ' ==================================================================================
        ' TRS_ReportGenerator_CashDebt_HTD
        ' ==================================================================================
        ' This business rule populates the CashDebtPosition_HTD_New.xlsx template with data
        ' from XFC_TRS_Master_CashDebtPosition table based on paramWeek and paramYear.
        ' 
                ...
                Select Case args.FunctionType
                    Case Is = ExtenderFunctionType.ExecuteDataMgmtBusinessRuleStep
                        ...
                        If Not String.IsNullOrEmpty(functionName) Then
                            If functionName.Equals("GenerateCashDebtHTDReport", StringComparison.InvariantCultureIgnoreCase) Then
                                Return GenerateCashDebtHTDReport(si, args)
                            End If
                        End If
                End Select
        ...
        Private Function GenerateCashDebtHTDReport(ByVal si As SessionInfo, ByVal args As ExtenderArgs) As Object
            Try
                ' ==========================================================================================
                ' 1. GET PARAMETERS
                ' ==========================================================================================
                ...
                Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
                    Dim allData As DataTable = Me.GetCashDebtData(si, dbConn, timeKey, expectedScenario, validWeek, previousWeek)
                    ...
                    Dim outputFileName As String = "CashDebtPosition_HTD_Report.xlsx"
                    ...
                    Using ms As New MemoryStream()
                        ...
                        Using doc As SpreadsheetDocument = SpreadsheetDocument.Open(ms, isEditable:=True)
                            ...
                            Me.UpdateCellValue(sheetData, "B10", "WEEK " & paramWeek & " - Start of " & mondayDate)
                            Me.UpdateCellValue(sheetData, "B40", "WEEK " & paramWeek & " - Start of " & endWeekFormatted)
                            Me.UpdateCellValue(sheetData, "B70", "WEEK " & paramWeek & " - Start Of " & eomFormatted)
