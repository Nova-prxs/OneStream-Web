...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Extender.TRS_ReportGenerator_CashFlowForecasting_HTD
	...
	Public Class MainClass
				...
				Select Case args.FunctionType
					Case Is = ExtenderFunctionType.ExecuteDataMgmtBusinessRuleStep
						...
						If Not String.IsNullOrEmpty(functionName) Then
							If functionName.Equals("GenerateCashFlowForecastingReport", StringComparison.InvariantCultureIgnoreCase) Then
								Return GenerateCashFlowForecastingReport(si, args)
							End If
						End If
				End Select
				Return Nothing
		...
		Private Function GenerateCashFlowForecastingReport(ByVal si As SessionInfo, ByVal args As ExtenderArgs) As Object
			Try
				' 1. Get Parameters
				...
				' Log the entry point
				BRApi.ErrorLog.LogMessage(si, $"GenerateCashFlowForecastingReport: Entrada en Semana {paramWeek} del año {paramYear}")

				' Calculate scenario: FW + week (2 digits with zero padding)
				Dim scenario As String = "FW" + paramWeek.ToString().PadLeft(2, "0"c)
				...
				' 3. Calculate 9-week window (4 before, current, 4 after)
				Dim weekWindow As New List(Of Integer)
				For i As Integer = -4 To 4
					Dim w As Integer = paramWeek + i
					If w >= 1 Then
						weekWindow.Add(w)
					End If
				Next
				...
				Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
					...
					Dim allDataSql As String = TRS_SQL_Repository.GetCashFlowActualGroupedByEntityWeekQuery(paramYear, Integer.Parse(paramWeek), True)
					...
					Using ms As New MemoryStream()
						...
						Using doc As SpreadsheetDocument = SpreadsheetDocument.Open(ms, isEditable:=True)
							...
							If titleRow IsNot Nothing Then
								SetCellStringValue(titleRow, "A", "HORSE HTD - Cashflow monitoring report SUMMARY", True)
								titleRow.Height = 21
						...
						Dim newFileInfo As New XFFileInfo(FileSystemLocation.ApplicationDatabase, "CashFlowSummary_HTD.xlsx", "Documents/Public/Treasury")
