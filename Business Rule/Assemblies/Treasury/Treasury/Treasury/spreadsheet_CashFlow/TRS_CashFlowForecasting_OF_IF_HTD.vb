...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Spreadsheet.TRS_CashFlowForecasting_OF_IF_HTD
	Public Class MainClass
			...
			Try
				Select Case args.FunctionType
					...
					Case Is = SpreadsheetFunctionType.GetTableView
						Return Me.GetCashFlowForecastingOFIFReport(si, args)
		...
		#Region "Cash Flow Forecasting OF/IF Report - HTD"

		Private Function GetCashFlowForecastingOFIFReport(ByVal si As SessionInfo, ByVal args As SpreadsheetArgs) As TableView
			Try
			...
			Dim horseGroupEntities As String() = {
				...
				Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
					...
					tv.Columns.Add(New TableViewColumn() With {.Name = "Entity", .Value = "Accum Operating + Investment Flows", .IsHeader = True})
				...
				' Header Row 2: Monday dates for each week with "Accum Operating + Investment Flows"
				Dim mondayDateRow As New TableViewRow() With {.IsHeader = True}
				mondayDateRow.Items.Add("Entity", New TableViewColumn() With {.Value = "Accum Operating + Investment Flows", .IsHeader = True})
				For Each weekNum As Integer In totalWeeks
				...
				tv.Rows.Add(mondayDateRow)					' ====================================
					' STEP 3: Get flow data for HTD entities and ALL weeks - ONLY Operating and Investment Flows
					' ====================================
					' Query for Actual data with proper filters
					' FILTER: Only Operating and Investment Flows + Actual data for current year
			...
			For Each weekNum As Integer In totalWeeks
				...
				' ====================================
				' STEP 6: Add "Total Horse Group" row (sum of all HTD entities)
				' ====================================
				...
				For Each weekNum As Integer In totalWeeks
					...
					Dim formattedValue As String = Me.FormatDecimalValue(totalValue)
