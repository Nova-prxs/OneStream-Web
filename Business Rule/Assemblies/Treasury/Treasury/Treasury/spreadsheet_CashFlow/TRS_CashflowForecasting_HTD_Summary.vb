...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Spreadsheet.TRS_CashflowForecasting_HTD_Summary
	Public Class MainClass
		'HTD Summary - Operating + Investment Flows by Entity (Rolling 4 Weeks)
		'Shows rolling 4-week sums of Inflows, Outflows, and Net Income for each HTD entity
			...
			Try
				Select Case args.FunctionType
					...
					Case Is = SpreadsheetFunctionType.GetTableView
						Return Me.GetHTDSummaryReport(si, args)
		...
		#Region "HTD Summary Report - Rolling 4 Weeks (Operating + Investment Flows)"

		Private Function GetHTDSummaryReport(ByVal si As SessionInfo, ByVal args As SpreadsheetArgs) As TableView
			Try
				...
				' HTD entities (exclude Aurobay Sweden, HPL, Aurobay China)
				...
				Using dbConn As DbConnInfo = BRApi.Database.CreateApplicationDbConnInfo(si)
					...
					weekNumberRow.Items.Add("Description", New TableViewColumn() With {.Value = $"HTD Summary - Scenario FW{paramWeek} (Rolling 4 Weeks)", .IsHeader = True})
					For Each weekNum As Integer In totalWeeks
					...
					' ====================================
					' STEP 3: Get individual week data (simplified approach like OF_IF_HTD)
					' ====================================
			...
			For Each row As DataRow In allDataDt.Rows
				...
				For Each htdEntity As String In htdEntities
					If String.Equals(entity, htdEntity, StringComparison.OrdinalIgnoreCase) Then
						matchedEntity = htdEntity
						Exit For
					End If
				Next
			...
			For Each entity As String In htdEntities
				...
				Next
			Next					' ====================================
					' STEP 5: Add entity rows (4 rows per entity: Name, Inflow, Outflow, Net Income)
					' ====================================
					For Each entity As String In htdEntities
