...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Spreadsheet.TRS_CashFlowForecasting_CF_CB_HTD_Average
	Public Class MainClass
		...
		Private Function GetYearAccumulatedValues(si As SessionInfo, dbConn As DbConnInfo, paramEntity As String, targetYear As String) As Dictionary(Of String, Decimal)
			Try
				Dim result As New Dictionary(Of String, Decimal) From {{"AccumulatedAll", 0}, {"AccumulatedOperInv", 0}}
				Dim yearInt As Integer
				If Not Integer.TryParse(targetYear, yearInt) Then Return result
				
				' Get max weeks for the target year
				Dim maxWeeksSql = TRS_SQL_Repository.GetMaxWeekInYearQuery(targetYear)
				Dim maxWeeksDt = BRApi.Database.ExecuteSql(dbConn, maxWeeksSql, False)
				...
				Dim entityFilter = If(paramEntity <> "HTD", $"AND Entity = '{paramEntity}'", "")
				Dim yearSql = TRS_SQL_Repository.GetCashFlowYearEntityQuery(targetYear, entityFilter, maxWeeks, True)
				Dim yearDt = BRApi.Database.ExecuteSql(dbConn, yearSql, False)
				Dim operInvAccounts = {"OPERATING FLOWS", "INVESTMENT FLOWS"}
			...
			If yearDt.Rows.Count = 0 Then
				If paramEntity = "HTD" Then
					For Each e In {"Horse Aveiro", "OYAK HORSE", "Horse Romania", "Horse Brazil", "Horse Chile", "Horse Argentina", "Horse Spain", "Horse Powertrain Solution"}
						result($"{e}_All") = 0
							result($"{e}_OperInv") = 0
						Next
					End If
				...
				For Each row As DataRow In yearDt.Rows
					...
					Dim account = If(IsDBNull(row("Account")), "", row("Account").ToString().ToUpper())
					...
					If Not entityWeekData.ContainsKey(entity) Then entityWeekData(entity) = New Dictionary(Of Integer, Dictionary(Of String, Decimal))
					If Not entityWeekData(entity).ContainsKey(weekNum) Then entityWeekData(entity)(weekNum) = New Dictionary(Of String, Decimal) From {{"All", 0}, {"OperInv", 0}}
					entityWeekData(entity)(weekNum)("All") += amount
					If operInvAccounts.Contains(account) Then entityWeekData(entity)(weekNum)("OperInv") += amount
				Next
				...
				For Each entityKvp In entityWeekData
					...
					Dim accAll = 0D
					Dim accOperInv = 0D
					For weekNum = 1 To maxWeeks
						If entityKvp.Value.ContainsKey(weekNum) Then
							accAll += entityKvp.Value(weekNum)("All")
							accOperInv += entityKvp.Value(weekNum)("OperInv")
						End If
					Next
					result($"{entityName}_All") = accAll
					result($"{entityName}_OperInv") = accOperInv
				Next
		...
		#Region "Cash Flow Forecasting Average Report - HTD"
		...
		Private Function GetCashFlowForecastingAverageReport(ByVal si As SessionInfo, ByVal args As SpreadsheetArgs) As TableView
			...
			Dim horseGroupEntities As String() = {
				...
				headerRow.Items.Add("Entity", New TableViewColumn() With {.Value = "Accum Cashflows + Cash Balance", .IsHeader = True})
					...
					' ====================================
					' STEP 3: Get flow data for HTD entities
					' ====================================
				...
				Next					' ====================================
					...
					' Process rows directly from unpivoted structure
